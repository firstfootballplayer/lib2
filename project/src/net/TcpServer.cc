...
#include "libmevent/net/Acceptor.h"
#include "libmevent/net/EventLoop.h"
#include "libmevent/net/EventLoopThreadPool.h"
#include "libmevent/net/SocketsOps.h"
...
TcpServer::TcpServer(EventLoop* loop, const InetAddress& listenAddr, const string& nameArg, Option option)
    : loop_(CHECK_NOTNULL(loop)), ipPort_(listenAddr.toIpPort()), name_(nameArg), acceptor_(new Acceptor(loop, listenAddr, option == kReusePort)), threadPool_(new EventLoopThreadPool(loop, name_)), connectionCallback_(defaultConnectionCallback), messageCallback_(defaultMessageCallback),
      nextConnId_(1)
{
    acceptor_->setNewConnectionCallback(std::bind(&TcpServer::newConnection, this, _1, _2));
}

TcpServer::TcpServer(const InetAddress& listenAddr, Option option) : loop_(nullptr), ipPort_(listenAddr.toIpPort()), name_("nameArg"), connectionCallback_(defaultConnectionCallback), messageCallback_(defaultMessageCallback), nextConnId_(1)
{
    loop_ = new EventLoop;
